    ...
    if os.path.exists(path):
        ...
        if data[1].endswith("k"):
            ...
            if data[1] == "best":
                ...
                vidcmd = f"yt-dlp -i --continue --embed-subs --no-warnings --prefer-ffmpeg -f 'bestvideo[ext=mp4][height<={data[1]}]+bestaudio[ext=m4a]/best' --playlist-start 1 --playlist-end 83 -o '{opdir}/%(playlist_index)s - %(title)s.%(ext)s' {url}"
