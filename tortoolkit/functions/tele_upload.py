...
async def upload_a_file(
    ...
    if user_msg is not None:
        ...
        if dis_thumb is False or dis_thumb is None:
            ...
            subprocess.call(f"ffmpeg -y -i '{path}' -c copy -attach '{thumb_path}' -metadata:s:t mimetype=image/jpeg -map 0 '{output_file_name}' ",shell=True)
...
async def upload_single_file(
    ...
    if user_msg is not None:
        ...
        if dis_thumb is False or dis_thumb is None:
            ...
            output_file_name = os.path.basename(path)
            subprocess.call(f"ffmpeg -y -i '{path}' -c copy -attach '{thumb_image_path}' -metadata:s:t mimetype=image/jpeg -map 0 '{output_file_name}' ",shell=True)
            os.replace(output_file_name,path)
